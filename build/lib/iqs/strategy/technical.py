...
from iqs.strategy.math_engine import (
...
def _strategy_me_simple_v1(df: pd.DataFrame, params: dict[str, Any]) -> dict[str, Any]:
    ...
    r_fit = r[-vol_window:]
    vol_cap = _clamp_float(params.get("vol_cap"), default=0.03)
    try:
    ...
    if vol_ok and h >= hurst_min and z_last >= entry_z:
        return {"signal": "BUY", "entry_price": px, "take_profit": px * (1.0 + tp_pct), "stop_loss": px * (1.0 - sl_pct)}
...
_STRATEGIES: dict[str, Any] = {"me_simple_v1": _strategy_me_simple_v1}
